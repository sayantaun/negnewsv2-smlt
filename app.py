...
def calc_new_col(row):
    if row['name_match'] == 'no': 
        ...
        if row['is_accused'] == 'no':
            m1 = "Subject not accused"
        else:
            m1 = ""
...
def check_neg_news(scraped_news,langchain_model,genai_model):
    neg_news = []
    pos_news = []
...
def apply_filters(neg_news,langchain_model, subject_name):
    ...
    for x in range(len(neg_news)):
        if len(r_filter) == 0:
            ...
            instrc1 = f"From the news text provided identify if the person {subject_name} is guilty or accused or convicted for a crime or charged with some illegal activities related to {topic_prompt} or faced compliance failures and anti-money laundering violations etc. Provide a truthful answer in yes or no. If not sure then say not sure."
            text = neg_news[x][4]
            seed_pattern = PromptPattern.from_str(instr1+" : {{text}}")
            template = seed_pattern.langchain.as_template()
            ...
            template = seed_pattern.langchain.as_template()
            responsec1 = langchain_model(template.format(text=text))
            ...
            if (response1 == "yes") and (responsec1 == "yes"):
                neg_news[x].extend([response1,responsec1, response2])
            ...
            instr1 = f"From the news text provided identify if the person {subject_name} is mentioned anywhere in the text. Provide a truthful answer in yes or no. If not sure then say not sure."
            instrc1 = f"From the news text provided identify if the person {subject_name} is guilty or accused or convicted for a crime or {subject_name} is charged with some illegal activities related to {topic_prompt} etc. Provide a truthful answer in yes or no. If not sure then say not sure."
            instr2 = f"From the news text provided identify if there is any mention of  {location} anywhere in the text or any state, city, county, village, suburbs, district of {location} mentioned anywhere in the text . Provide a truthful answer in yes or no. If not sure then say not sure."
            ...
            if (response1 == "yes") and (responsec1 == "yes") and (response2 == "yes"):
                vmatch = 1
                neg_news[x].extend([response1,responsec1, response2])
                tp.append(neg_news[x])
            else:
...
def report_fp(fp,langchain_model,subject_name):
    ...
    for x in range(len(fp)) :
        ...
        if (response_name == "yes"):
            ...
            summ_name = "Subject is missing in the news." 
        summary = response.rstrip(".") +"."+ summ_name
        fp_results_ll = [fp[x][1],fp[x][3],summary,fp[x][5],fp[x][6],fp[x][7],fp[x][8]]
...
def report_tp(tp,langchain_model,subject_name):
    ...
    for x in range(len(tp)) :
        ...
        tp_results_ll = [tp[x][1],tp[x][3],summary,tp[x][5],tp[x][6],tp[x][7],tp[x][8]]
...
def  final_conclusion(tp,fp, pos_news,subject_name, num_results):
    ...
    conclusion_text_general = "Neg-News-"+str(ctp)+"  Un-related News-"+str(cfp)+" "
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button('Process'):
            with st.spinner("Processing"):
                ...
                print("step4")
                neg_news, pos_news =  check_neg_news(scraped_news,langchain_model_classify,genai_model)
                print("step5")
    ...
    if st.button('Generate'):
        with st.spinner("Processing"):
            ...
            if len(r_tp) > 0:
                df_tp = pd.DataFrame(r_tp, columns =['Url', 'Published Date','Summary','Topic','name_match','is_accused','residence_match'])
            ...
            if len(r_fp) > 0:
                df_fp = pd.DataFrame(r_fp, columns =['Url', 'Published Date','Summary','Topic','name_match','is_accused','residence_match'])
            ...
            tab1, tab2, tab5, tab4 = st.tabs(["Negative News", "Un-related results", "Screening Conclusion", "Bad URLs (for manual check)"])
